...
class Setup
{
    ...
    public static function enqueueScriptsAndStyles()
    {
        ...
        if( is_admin() && isset( $screen->post_type ) && 'cupom_gerado' == $screen->post_type ) {

            // Load only for Add Post and Edit Post screens
            if( 'cupom_gerado' == $screen->post_type && ( ( isset( $screen->action ) && $screen->action == 'add' ) || ( isset( $screen->base ) && 'cupom_gerado' == $screen->base && empty( $screen->action ) ) ) ) {
